...
def obtain_ion_name(pdb_name):
    ion_names = []
    file_handle = open('/home/yxguo/project/DockingDB/pdbs_classified/single_ligand_pdb/{}.pdb'.format(pdb_name))
    content = file_handle.readlines()
    hetnam_lines = [line for line in content if re.search("^HETNAM ", line)]
    ion_lines = [line for line in hetnam_lines if re.search(" ION$", line)]
...
def obtain_ligand_serial(pdb_name):
    ...
    for het_line in het_lines:
        if het_line.split()[1] not in modres_list:
            chain = het_line[12]
...
def dock_it(pdb_name):
    ...
    os.system("cp /home/yxguo/project/DockingDB/pdbs_classified/single_ligand_pdb/{}.pdb ./".format(pdb_name))
